...
class Mitsuba(object):
        ...
        resctScreen = pmgr.create({
                    ...
                    'bsdf': {
                        'type': 'diffuse',
                            ...
                            #'reflectance' : Spectrum(0.78) # this is causing peaks noise
    ...
    def createSampler(self,sampleCount):
        pmgr = PluginManager.getInstance()
        obj = pmgr.create({
                    'type' : 'ldsampler',#''independent',#'sobol',#'ldsampler',  #',
    ...
    def Render(self,sampleCount,i):
        ...
        currScene.setSampler(self.sampler) #O to sleep :-) who is it?!!? the void hahahah AMIT ?!?!(self.sampler)

        currScene.setDestinationFile('')
        # Create a render job and insert it into the queue
        #job = RenderJob('myRenderJob'+str(i), currScene, self.queue )
        job = RenderJob('myRenderJob'+str(i), currScene, self.queue,self.sceneResID )
        job.start()
        ...
        result_image = np.array(bitmap.buffer())                                
